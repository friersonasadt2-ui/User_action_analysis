...
#ui_buy 是历史购买次数，不是未来标签，但能说明“越近期越可能买”的规律性
rec <- ui_features[, .(ui_recency_hours, ui_buy)]

#过滤
rec <- rec[is.finite(ui_recency_hours)]
rec[, rec_bin := cut(ui_recency_hours,
                     breaks = c(-Inf, 6, 24, 72, 168, 336, Inf),
                     labels = c("<=6h","6-24h","1-3d","3-7d","7-14d",">14d"),
                     right = TRUE)]
bin_df <- rec[, .(
  n_pairs = .N,
  buy_rate_pairs = mean(ui_buy > 0)  # 发生过购买的(user,item)比例
...
p4 <- ggplot(bin_df, aes(x = rec_bin, y = buy_rate_pairs, fill = buy_rate_pairs)) +
  ...
  labs(title = "Recency vs Purchase",
