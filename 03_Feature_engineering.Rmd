...
```{r}
dt <- fread("output/user_action_clean.csv", na.strings = c("", "NA", "NaN", "NULL"))
...
fwrite(feat, "output/features_user_item_raw.csv")
...
#raw总表（含 split）
fwrite(feat, "output/all_set_raw.csv")
...
fwrite(feat, "output/all_set_processed.csv")
#输出建模用数据集
fwrite(train_dt, "output/train_set.csv")
fwrite(val_dt,   "output/val_set.csv")
fwrite(test_dt,  "output/test_set.csv")
...
fwrite(feat_dict, "output/feature_dictionary.csv", bom = TRUE)
