...
```{r}
train_dt <- fread("output/train_set.csv")
val_dt   <- fread("output/val_set.csv")
test_dt  <- fread("output/test_set.csv")
all_dt   <- fread("output/all_set_processed.csv")
...
fwrite(corr_pairs, "output/feature_corr_pairs.csv")
...
fwrite(data.table(feature = nzv),          "output/feature_drop_zero_var.csv")
fwrite(data.table(feature = to_drop_corr), "output/feature_drop_by_corr.csv")
fwrite(data.table(feature = keep_cols),    "output/feature_keep_final.csv")
...
fwrite(train_keep, "output/train_set_filtered.csv")
fwrite(val_keep,   "output/val_set_filtered.csv")
fwrite(test_keep,  "output/test_set_filtered.csv")
fwrite(all_keep,   "output/all_set_filtered.csv")

#过滤特征字典+按 dict_keep_features固定顺序排序
feat_dict <- fread("output/feature_dictionary.csv", encoding = "UTF-8")
...
fwrite(feat_dict_filtered, "output/feature_dictionary_filtered.csv", bom = TRUE)
