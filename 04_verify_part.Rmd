...
---
#功能：
#对特征与模型效果进行验证评估，输出性能指标与稳定性结论
#数据集划分与验证策略（留出法/交叉验证/时间切分）
#特征筛选（低方差、高缺失、高相关冗余、单变量区分度如 AUC/IV 等）
#输出验证结论与特征删减
...
#去掉零方差和常数特征
X0 <- as.matrix(train_dt[, ..feat_cols])
var_vec <- matrixStats::colVars(X0, na.rm = TRUE)
names(var_vec) <- feat_cols
nzv <- names(var_vec)[!is.finite(var_vec) | var_vec == 0]
...
lab_cor <- suppressWarnings(cor(X, y, use = "pairwise.complete.obs"))
lab_cor[!is.finite(lab_cor)] <- 0
lab_cor <- abs(lab_cor)
names(lab_cor) <- colnames(X)

var_map <- matrixStats::colVars(X, na.rm = TRUE)
var_map[!is.finite(var_map)] <- 0
names(var_map) <- colnames(X)
...
if (nrow(corr_pairs) > 0) {
  for (i in seq_len(nrow(corr_pairs))) {
    ...
    sa <- lab_cor[a]; if (is.na(sa)) sa <- 0
    sb <- lab_cor[b]; if (is.na(sb)) sb <- 0
    ...
    if (sa > sb) {
      ...
      va <- var_map[a]; if (is.na(va)) va <- 0
      vb <- var_map[b]; if (is.na(vb)) vb <- 0
...
# 尽量释放旧对象
rm_list <- intersect(c(
  "uni_tbl","out_tbl","tmp_rank","corr_pairs","C","X","X0",
  "df_glm","d2","sm","imp_logit","perm_imp",
  "auc_vals","iv_vals"
), ls())
if (length(rm_list)) rm(list = rm_list)
invisible(gc())
...
if (zero_bin && x0_ratio >= zero_bin_ratio && any(tmp$x == 0)) {
  ...
  #WOE/IV
  tab <- tmp[, .(good = sum(y == 0L), bad = sum(y == 1L)), by = bin]
  tab[, `:=`(good = as.numeric(good), bad = as.numeric(bad))]
  tab[good == 0, good := eps]
  tab[bad  == 0, bad  := eps]
  ...
  tab[, woe := log(dist_bad / dist_good)]
  tab[, iv_part := (dist_bad - dist_good) * woe]
  iv <- sum(tab$iv_part, na.rm = TRUE)

  list(iv = as.numeric(iv), detail = tab[order(bin)])
}
...
auc_vals <- vapply(feat_cols, function(cn) fast_auc(train_dt[[cn]], y), numeric(1))
iv_vals  <- vapply(feat_cols, function(cn) calc_iv_woe(train_dt, cn, y_col = label_col)$iv, numeric(1))

univ_tbl <- data.table(
  feature = feat_cols,
  auc = as.numeric(auc_vals),
  iv = as.numeric(iv_vals)
)

#输出到 validation_uni
out_tbl <- univ_tbl[, .(feature, auc, iv)]
out_tbl[is.na(auc), auc := -Inf]
setorder(out_tbl, -auc)
fwrite(out_tbl, file.path(dir_uni, "univariate_auc_iv.csv"))
rm(out_tbl)
gc()


#特征重要性：Logistic 回归
topK <- min(35L, nrow(univ_tbl))
feat_for_logit <- univ_tbl[order(-auc)][1:topK, feature]
...
drop_feats <- c(
  #单变量几乎无信息(AUC≈0.50 且 IV≈0)
  "i_fav_1d","i_fav_2d","i_fav_3d","i_fav_4d","i_fav_5d","i_fav_6d","i_fav_7d",
  "ui_fav_1d","ui_fav_2d","ui_fav_3d","ui_fav_4d","ui_fav_5d","ui_fav_6d","ui_fav_7d",
  "ui_fav_rate",

  #贡献很弱且冗余高(短窗口到长窗口高度共线)
  "i_cart_1d","i_cart_2d","i_cart_3d","i_cart_4d",
  "ui_cart_1d","ui_cart_2d","ui_cart_3d","ui_cart_4d","ui_cart_5d","ui_cart_6d",

  #在logit+perm里边际贡献接近0/为负，且有更强替代
  "i_cart_6d","i_cart_7d","i_buy_4d","i_buy_5d","i_buy_6d","i_buy_7d",
  "u_buy_7d","i_view_4d","ui_view_5d","ui_view_6d"
)
...
feat_dict2 <- data.table::fread(file.path(dir_dict,"feature_dictionary_filtered.csv"), encoding = "UTF-8")
